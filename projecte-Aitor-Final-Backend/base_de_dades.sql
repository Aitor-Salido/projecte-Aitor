...
-- Tiempo de generación: 15-01-2026
...
--
-- Base de datos: `garatge_virtual`
--
DROP DATABASE IF EXISTS `garatge_virtual`;
CREATE DATABASE IF NOT EXISTS `garatge_virtual` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `garatge_virtual`;

-- --------------------------------------------------------

--
-- 1. Tabla USUARI (Con seguridad SHA-256)
--
...
CREATE TABLE `usuari` (
  ...
  `contrasenya` char(64) NOT NULL COMMENT 'Hash SHA-256 (64 caracteres)',
  `data_registre` date DEFAULT curdate(),
  `imatge_perfil` varchar(255) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

INSERT INTO `usuari` (`id_usuari`, `nom`, `email`, `contrasenya`, `data_registre`, `imatge_perfil`) VALUES
(1, 'Aitor', 'aitor@example.com', '03ac674216f3e15c761ee1a5e255f067953623c8b388b4459e13f978d7c846f4', '2025-11-05', 'img/aitor.png');

-- --------------------------------------------------------

--
-- 2. Tabla VEHICLE (Con tipos de combustible)
--
...
--
-- 3. Tabla CONSUM (Adaptada para Litros y/o kWh)
--
...
CREATE TABLE `consum` (
  ...
  -- consum_mitja se calculará automáticamente en las vistas, pero lo dejamos por si quieres guardar el dato del ordenador de a bordo
...
-- Coche 1 (Hibrido): Gastó gasolina y un poco de electricidad
(1, 1, '2025-10-01', 45.50, 12.00, 85.00, 15000, 6.5),
-- Coche 2 (Diesel): Solo litros
(2, 2, '2025-09-15', 75.00, NULL, 115.00, 290000, 9.5),
-- Coche 3 (Eléctrico): Solo kWh
(3, 3, '2025-10-03', NULL, 55.00, 18.00, 10500, 16.2);

-- --------------------------------------------------------

--
-- 4. Tablas MANTENIMENT y REPARACIONS
--
...
--
-- 5. VISTAS DE RANKINGS (La parte mágica)
--

-- A. Ranking Consumo GRUPO 1: Térmicos (Gasolina, Diesel, Gas) -> Ordena por Litros
...
-- B. Ranking Consumo GRUPO 2: Eléctricos -> Ordena por kWh
...
-- C. Ranking Consumo GRUPO 3: Híbridos -> Muestra Litros y kWh
...
SELECT 
    ...
    -- Aquí usamos el consumo medio del ordenador como referencia principal
...
-- D. Ranking de MANTENIMIENTO (Solo costes de mantenimiento)
...
-- E. Ranking de REPARACIONES (Solo costes de averías)
...
-- --------------------------------------------------------
-- Otras tablas (Forum, ITV, Recordatori) sin cambios
-- --------------------------------------------------------

CREATE TABLE `forum` (
  `id_post` int(11) NOT NULL,
  `id_usuari` int(11) NOT NULL,
  `id_vehicle` int(11) DEFAULT NULL, -- Nuevo campo: Puede estar vacío si el post es general
...
ALTER TABLE `forum` ADD PRIMARY KEY (`id_post`), ADD KEY `id_usuari` (`id_usuari`);
...
ALTER TABLE `usuari` MODIFY `id_usuari` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
ALTER TABLE `forum` ADD CONSTRAINT `forum_ibfk_1` FOREIGN KEY (`id_usuari`) REFERENCES `usuari` (`id_usuari`) ON DELETE CASCADE;
...
ALTER TABLE `forum` ADD CONSTRAINT `forum_ibfk_2` FOREIGN KEY (`id_vehicle`) REFERENCES `vehicle` (`id_vehicle`) ON DELETE SET NULL;
